...
public class MapDataProviderRw<TFrom, TTo> : ISourceDataProviderRw<TTo>
    where TFrom : IEntity
    where TTo : IEntity
{
    ...
    public MapDataProviderRw(IDataProviderRw<TFrom> provider, IConfigurationProvider config, IMapper mapper)
    {
        _provider = provider;
        _config = config;
        _mapper = mapper;
