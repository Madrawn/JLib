...
public static class ExpressionHelper
{
    ...
    public static Expression<Func<TSource, TKey?>> ToNullable<TSource, TKey>(this Expression<Func<TSource, TKey>> expression)
        where TKey : struct
    {
    ...
    public static PropertyInfo GetPropertyInfo<TSource, TValue>(this Expression<Func<TSource, TValue>> propertyLambda)
    {
        ...
        var propType = tValue.IsGenericType && tValue.GetGenericTypeDefinition() == typeof(Nullable<>) ? tValue.GenericTypeArguments.First() : tValue;

        if (type != propInfo.ReflectedType && (propInfo.ReflectedType.IsInterface && !type.Implements(propInfo.ReflectedType))
